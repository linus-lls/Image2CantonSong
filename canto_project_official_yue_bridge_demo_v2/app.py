...
if st.session_state["step_2_done"]:
    ...
    with st.expander("Evaluation", expanded=False):
        ...
        with eval_tabs[0]:
            ...
            if st.session_state.get("image_lyrics_alignment") is not None:
                ...
                with st.expander("Debug log", expanded=False):
                    st.code(st.session_state["image_lyrics_alignment_error"], language="python")
        ...
        with eval_tabs[1]:
            ...
            st.caption(
                f"Image model 25cat supports up to {max_image_labels} classes. "
                f"Text model {text_emotion_model} supports up to {max_text_labels} classes. "
                "Choose top-k accordingly."
            )
            top_k_image_eval = st.number_input(
                "Top-k image emotions",
                min_value=1,
                max_value=max_image_labels,
                value=min(5, max_image_labels),
                step=1,
            ...
            if st.session_state.get("image_lyrics_emotion_similarity") is not None:
                ...
                if image_preds:
                    ...
                    for item in image_preds:
                        ...
                        if english:
                            st.write(f"- {item['label']} ({english}): {item['score']:.4f}")
                        else:
                ...
                with st.expander("Debug log", expanded=False):
                    st.code(st.session_state["image_lyrics_emotion_similarity_error"], language="python")
